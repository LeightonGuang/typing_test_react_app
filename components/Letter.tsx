...
const Letter = ({ char, isCorrect, isActive }: Props) => {
  ...
  return (
    ...
    >
      <span
        className={`${isActive ? "absolute" : "hidden"} ${caret === "block" && "inset-0 bg-input opacity-50"} ${caret === "bar" && "h-full w-full border-l-2 border-foreground"} ${caret === "underline" && "h-full w-full border-b-2 border-foreground"}`}
      />
      {char}
    </div>
  );
